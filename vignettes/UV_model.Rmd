---
title: "Case study: Modelling the unimproved value in the ACT's Woden Valley"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Case study: Modelling the unimproved value in the ACT's Woden Valley}
...
The goal of this case study is to show how to use `allhomes` to pull in past sales data for various ACT suburbs and then fit a mixed-effect model using Stan to predict the unimproved value (UV, i.e. the value of the plot of land itself) as a function of the block size (in square-meters), the year of the sale and the suburb. We then explore the output of the model and interpret its results.
...
We inspects pairwise correlations between the fixed-effect parameter estimates in bivariate scatter plots. This is useful for identifying divergencies, collinearities and multiplicative non-identifiabilities.

```{r pairs, fig.height = 4, fig.width = 7}
...
```{r loo-r-squared, fig.height = 4, fig.show="hold", out.width="47%"}
...
```{r forecast-2020, cache=TRUE}
data_pred_2020 <- data_model %>%
    data_grid(
...
We draw the median and 95% quantile intervals of the UV predictions as a function of block size for every division (suburb) and each of the three years. The red dots are the recorded UVs for 2020.

```{r plot-forecast-2020, fig.width = 7, fig.height = 7}
data_pred_2020 %>%
...
1. Properties of around 1000 $m^2$ have the highest UV in the suburbs Phillip, Swinger Hill and Garran. 
    ...
    Lyons, Mawson and Torrens properties of the same size have the lowest UV.
    
    ```{r table-min-UV, echo=FALSE}
    UV_1000sqm_2020 %>% tail(3)
    ```
    
2. The suburbs Phillip, Swinger Hill and Hughes (are predicted to) show the largest change in UV between 2010 and 2030; the suburbs Torrens, Mawson and Lyons show the smallest change in UV during that period.
