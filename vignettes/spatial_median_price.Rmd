---
title: "Case study: Variability of the unimproved value per sqm across the ACT"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Case study: Variability of the unimproved value per sqm across the ACT}
...
- [`absmapsdata`](https://github.com/wfmackey/absmapsdata) for getting ABS geometries of [SA2 regions](https://www.abs.gov.au/statistics/standards/australian-statistical-geography-standard-asgs-edition-3/jul2021-jun2026/main-structure-and-greater-capital-city-statistical-areas/statistical-area-level-2) (which -- within cities -- usually represent suburbs), and
...
We obtain 2021 statistical area 2 (SA2) geospatial data provided by the ABS through `absmapsdata`, and only keep records for areas in the ACT. SA2 names sometimes include the territory name, so we clean names by removing the territory name if present. We then store all cleaned SA2 names in preparation for using these names in the past sales `allhomes` search.

```{r spatial-data-ACT, cache=TRUE}
...
We now get past sales data for all suburbs as given in `sa2_names` for the years 2021 and 2022. Since `get_past_sale_data()` requires suburbs to be specified in format "suburb_name, state/territory_abbrevation", we append `", ACT"` to entries in `sa2_names`. This process may take a few minutes.
